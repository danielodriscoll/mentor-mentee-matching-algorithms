...
def initialize_population(pop_size, n_mentees, n_mentors, similarity_matrix, max_mentees_per_mentor=2):
    """
    ...
    One-sided version: Only considers mentee preferences when assigning, 
    but still respects mentor capacity.
    """
    population = []
    ...
    for _ in range(pop_size):
        ...
        # Create a random permutation of mentees to avoid bias
        ...
        for mentee_idx in mentee_order:
            ...
            # ONE-SIDED CHANGE: We only consider mentee's preferences here
            mentor_similarities = similarity_matrix[mentee_idx]
            
            # Sort mentors by preference (highest to lowest similarity)
...
def fitness_function(chromosome, similarity_matrix, exact_match_mentors, exact_match_mentees, 
    ...
    """
    Modified fitness function for one-sided matching.
    Focuses primarily on mentee satisfaction.
    """
    ...
    for mentee_idx in range(n_mentees):
        ...
        # ONE-SIDED CHANGE: We care mostly about mentee's preference for the mentor
    ...
    # ONE-SIDED CHANGE: Put more weight on mentee satisfaction (similarity)
    similarity_weight = 100
    validity_weight = 1 + (len(constraints) / 10)  # Still increases with more constraints
    
    fitness = (validity_percentage * validity_weight) + (avg_similarity * similarity_weight)
...
def mutation(chromosome, n_mentors, similarity_matrix, mutation_rate=0.05):
    ...
    One-sided version focuses on assigning mentees to their preferred mentors.
    """
    n_mentees = len(chromosome)
    ...
    for mentee_idx in range(n_mentees):
        ...
        if random.random() < mutation_rate:
            ...
            # Get mentor preferences for this mentee based on similarity
            # ONE-SIDED CHANGE: We prioritize mentee's preference when mutating
            mentor_similarities = similarity_matrix[mentee_idx]
            sorted_mentor_indices = np.argsort(-mentor_similarities)
            
            # Try to reassign to one of the top mentors by mentee preference
...
def genetic_algorithm(mentors_df, mentees_df, exact_match_mentors, exact_match_mentees, features,
    ...
    One-sided version that prioritizes mentee preferences.
    """
    ...
    # ONE-SIDED CHANGE: Simplified initialization that focuses on mentee preferences
    population = initialize_population(
        pop_size, n_mentees, n_mentors, similarity_matrix, 
