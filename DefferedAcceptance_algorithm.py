...
def deferred_acceptance_matching(mentors_df, mentees_df, max_mentees_per_mentor=2, constraints=None):
    """
    Implements the Gale-Shapley Deferred Acceptance algorithm for mentor-mentee matching.
    This is a one-sided version where mentees propose to mentors.
    
    The algorithm proceeds as follows:
    1. Each mentee has a ranked list of preferred mentors
    2. In each round, all unmatched mentees propose to their most preferred mentor
       who hasn't yet rejected them
    3. Each mentor tentatively accepts proposals up to their capacity limit,
       rejecting any others
    4. The process repeats until all mentees are matched or have been rejected by
       all possible mentors
    """
    ...
    no_matches_possible = False
    
    # Maximum iterations to prevent infinite loops in case of implementation issues
    ...
    while unmatched_mentees and iteration < max_iterations:
        ...
        for mentor_idx in mentee_preferences[mentee_idx]:
            ...
            current_matches = len(mentor_matches[mentor_idx])
            max_capacity = constraints.get('max_mentees_per_mentor', max_mentees_per_mentor)
            
            if current_matches < max_capacity:
        ...
        if not found_mentor:
            # Check if mentee has proposed to all possible mentors
    ...
    for mentor_idx, mentee_indices in mentor_matches.items():
        for mentee_idx in mentee_indices:
            ...
            distance = np.linalg.norm(mentor_features - mentee_features)
            max_distance = np.sqrt(len(features))  # Maximum possible distance
...
def randomized_matching_experiment(mentors_df, mentees_df, num_randomizations=100, max_mentees_per_mentor=2, constraints=None):
    """
    Run the Deferred Acceptance mentor-mentee matching algorithm across multiple randomized datasets.
    """
    ...
    print(f"\nRunning {num_randomizations} randomized iterations of the Deferred Acceptance matching algorithm...")
    ...
    for i in range(num_randomizations):
        ...
        matches_df = deferred_acceptance_matching(
            shuffled_mentors, 
            shuffled_mentees, 
...
def run_experiment(mentors_file, mentees_file, num_randomizations=100, max_mentees_per_mentor=2, 
    ...
    print("\n" + "="*50)
    print(f"DEFERRED ACCEPTANCE ALGORITHM RESULTS ({num_randomizations} RANDOMIZED ITERATIONS)")
    print("="*50)
    ...
    output_path = 'data/deferred_acceptance_results.csv'
    if mentor_sample_size != 500 or mentee_sample_size != 1000:
        # This is a scalability test
        output_path = f'data/deferred_acceptance_{mentor_sample_size}m_{mentee_sample_size}m.csv'
    elif len(constraints) > 2:
        # This is a constraint test
        constraint_count = len(constraints)
        output_path = f'data/deferred_acceptance_constraints_{constraint_count}.csv'
...
if __name__ == "__main__":
    try:
        print("Starting Deferred Acceptance mentor-mentee matching experiment...")
