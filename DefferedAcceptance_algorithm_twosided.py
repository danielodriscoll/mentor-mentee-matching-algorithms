...
def calculate_mentee_preferences(mentors_df, mentees_df):
    """
    Calculate preference lists for each mentee based on similarity to mentors.
    Returns a dictionary where each mentee has a ranked list of mentors.
    """
    features = [col for col in mentors_df.columns if col != 'User_ID']
    
    mentor_array = mentors_df[features].values
    mentee_array = mentees_df[features].values
    
    # Calculate distances
    distances = euclidean_distances(mentee_array, mentor_array)
    
    # Create preference lists (smaller distance = higher preference)
    mentee_preferences = {}
    for mentee_idx in range(len(mentees_df)):
        # Sort mentors by distance (ascending) to get preference order
        ranked_mentors = np.argsort(distances[mentee_idx])
        mentee_preferences[mentee_idx] = ranked_mentors.tolist()
    
    return mentee_preferences

def calculate_mentor_preferences(mentors_df, mentees_df):
    """
    Calculate preference lists for each mentor based on similarity to mentees.
    Returns a dictionary where each mentor has a ranked list of mentees.
    """
    features = [col for col in mentors_df.columns if col != 'User_ID']
    ...
    # Calculate distances from mentors to mentees
    distances = euclidean_distances(mentor_array, mentee_array)
    
    # Create preference lists (smaller distance = higher preference)
    mentor_preferences = {}
    for mentor_idx in range(len(mentors_df)):
        # Sort mentees by distance (ascending) to get preference order
        ranked_mentees = np.argsort(distances[mentor_idx])
        mentor_preferences[mentor_idx] = ranked_mentees.tolist()
...
def meets_all_constraints(mentors_df, mentees_df, mentor_idx, mentee_idx, constraints):
    ...
    if constraints.get('secondary_language_match', False):
        try:
            ...
            # Skip this constraint if columns are missing or data format is unexpected
            pass
...
def two_sided_deferred_acceptance(mentors_df, mentees_df, max_mentees_per_mentor=2, constraints=None):
    """
    Implements the two-sided Gale-Shapley Deferred Acceptance algorithm for mentor-mentee matching.
    In this version, both mentor and mentee preferences are considered.
    
    The algorithm proceeds as follows:
    1. Calculate preference rankings for both mentees and mentors
    2. Mentees propose to mentors in order of their preferences
    3. Mentors tentatively accept or reject based on their own preferences
       and current matches
    4. The process repeats until stability is reached
    """
    ...
    # Calculate preferences for both sides
    mentee_preferences = calculate_mentee_preferences(mentors_df, mentees_df)
    mentor_preferences = calculate_mentor_preferences(mentors_df, mentees_df)
    
    # Initialize data structures
    n_mentees = len(mentees_df)
    n_mentors = len(mentors_df)
    
    # Current matches (mentor_idx -> list of mentee_idx)
    mentor_matches = defaultdict(list)
    
    # For each mentor, track the rank they assign to each mentee
    mentor_ranks = {}
    for mentor_idx, prefs in mentor_preferences.items():
        mentor_ranks[mentor_idx] = {mentee_idx: rank for rank, mentee_idx in enumerate(prefs)}
    
    # Queue of unmatched mentees
    unmatched_mentees = deque(range(n_mentees))
    
    # Track which mentors each mentee has already proposed to
    mentee_proposals = [set() for _ in range(n_mentees)]
    
    # Maximum iterations to prevent infinite loops
    max_iterations = n_mentees * n_mentors
    iteration = 0
    
    # Run the two-sided deferred acceptance algorithm
    while unmatched_mentees and iteration < max_iterations:
        iteration += 1
        
        # Get the next unmatched mentee
        mentee_idx = unmatched_mentees.popleft()
        
        # Find next preferred mentor who hasn't been proposed to yet
        found_mentor = False
        for mentor_idx in mentee_preferences[mentee_idx]:
            # Skip if already proposed to this mentor
            if mentor_idx in mentee_proposals[mentee_idx]:
                continue
                
            # Mark that this mentee has proposed to this mentor
            mentee_proposals[mentee_idx].add(mentor_idx)
            
            # Check if this mentor-mentee pair meets all constraints
            if not meets_all_constraints(mentors_df, mentees_df, mentor_idx, mentee_idx, constraints):
                continue
            
            # Check if mentor has capacity or if this mentee is preferred to current matches
            current_matches = mentor_matches[mentor_idx]
            max_capacity = constraints.get('max_mentees_per_mentor', max_mentees_per_mentor)
            
            if len(current_matches) < max_capacity:
                # Mentor has capacity, accept the proposal
                mentor_matches[mentor_idx].append(mentee_idx)
                found_mentor = True
                break
            else:
                # Mentor is at capacity, check if this mentee is preferred to any current match
                # Get the rank of this mentee from the mentor's perspective
                mentee_rank = mentor_ranks[mentor_idx].get(mentee_idx, float('inf'))
                
                # Get the worst current match
                worst_match_idx = None
                worst_match_rank = -1
                
                for current_mentee_idx in current_matches:
                    current_rank = mentor_ranks[mentor_idx].get(current_mentee_idx, float('inf'))
                    if current_rank > worst_match_rank:
                        worst_match_rank = current_rank
                        worst_match_idx = current_mentee_idx
                
                # If this mentee is preferred to the worst current match, replace it
                if mentee_rank < worst_match_rank:
                    mentor_matches[mentor_idx].remove(worst_match_idx)
                    mentor_matches[mentor_idx].append(mentee_idx)
                    # Put the rejected mentee back in the queue
                    unmatched_mentees.append(worst_match_idx)
                    found_mentor = True
                    break
        
        # If mentee couldn't find any suitable mentor, put back in the queue
        # only if they haven't proposed to all possible mentors
        if not found_mentor and len(mentee_proposals[mentee_idx]) < n_mentors:
            unmatched_mentees.append(mentee_idx)
        
        # Check for early termination - if all mentees have proposed to all mentors
        # and we still have unmatched mentees
        all_proposed = True
        for mentee_idx in unmatched_mentees:
            if len(mentee_proposals[mentee_idx]) < n_mentors:
                all_proposed = False
                break
        
        if all_proposed and unmatched_mentees:
            print(f"⚠️ No more matches possible with current constraints. Terminating early.")
            break
    
    if iteration >= max_iterations:
        print(f"⚠️ Reached maximum iterations ({max_iterations}). Algorithm may not have converged.")
    
    # Convert matches to the expected format
    matches = []
    for mentor_idx, mentee_indices in mentor_matches.items():
        for mentee_idx in mentee_indices:
            # Calculate similarity
            mentor_features = mentors_df.iloc[mentor_idx][features].values
            mentee_features = mentees_df.iloc[mentee_idx][features].values
            
            # Calculate a combined similarity that considers both perspectives
            # Mentee's preference for mentor
            mentee_pref_rank = mentee_preferences[mentee_idx].index(mentor_idx)
            mentee_pref_score = 1 - (mentee_pref_rank / n_mentors)
            
            # Mentor's preference for mentee
            mentor_pref_rank = mentor_preferences[mentor_idx].index(mentee_idx)
            mentor_pref_score = 1 - (mentor_pref_rank / n_mentees)
            
            # Calculate weighted similarity (70% mentee preference, 30% mentor preference)
            similarity = 0.7 * mentee_pref_score + 0.3 * mentor_pref_score
    ...
    # Early termination check - if no matches were made
    if not matches:
        print(f"⚠️ No matches possible with current constraints. Terminating early.")
...
def randomized_matching_experiment(mentors_df, mentees_df, num_randomizations=100, max_mentees_per_mentor=2, constraints=None):
    """
    Run the Two-Sided Deferred Acceptance mentor-mentee matching algorithm across multiple randomized datasets.
    """
    ...
    print(f"\nRunning {num_randomizations} randomized iterations of the Two-Sided Deferred Acceptance matching algorithm...")
    ...
    for i in range(num_randomizations):
        ...
        matches_df = two_sided_deferred_acceptance(
            shuffled_mentors, 
            shuffled_mentees, 
        ...
        # Early termination if multiple iterations produce no matches
        if results['total_matches'] == 0:
            print(f"⚠️ Iteration {i+1}: No matches possible with current constraints")
            if i >= 2 and all(r['total_matches'] == 0 for r in all_results):
                print(f"⚠️ Multiple iterations with no matches. Constraints appear too restrictive. Terminating experiment.")
                break
...
def run_experiment(mentors_file, mentees_file, num_randomizations=100, max_mentees_per_mentor=2, 
    ...
    print("\n" + "="*50)
    print(f"TWO-SIDED DEFERRED ACCEPTANCE ALGORITHM RESULTS ({num_randomizations} RANDOMIZED ITERATIONS)")
    print("="*50)
    ...
    output_path = 'data/twosided_deferred_acceptance_results.csv'
    if mentor_sample_size != 500 or mentee_sample_size != 1000:
        # This is a scalability test
        output_path = f'data/twosided_deferred_acceptance_{mentor_sample_size}m_{mentee_sample_size}m.csv'
    elif len(constraints) > 2:
        # This is a constraint test
        constraint_count = len(constraints)
        output_path = f'data/twosided_deferred_acceptance_constraints_{constraint_count}.csv'
...
if __name__ == "__main__":
    try:
        print("Starting Two-Sided Deferred Acceptance mentor-mentee matching experiment...")
