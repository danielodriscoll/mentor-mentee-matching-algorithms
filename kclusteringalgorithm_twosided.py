...
def encode_features(df, features):
    ...
    for feature in features:
        ...
        encoded[feature] = encoders[feature].fit_transform(df[feature].astype(str))
...
def preprocess_data(mentors_file, mentees_file, mentor_sample_size=500, mentee_sample_size=1000):
    ...
    feature_weights = {
        ...
        'Primary_Language': 1.4,
        'Industry': 1.4,
        'Availability': 1.1,
        'Communication_Methods': 1.2,
        'Mentoring_Preferences': 1.5,
        'Education': 1.3
    }
    
    # One-hot encode both mentors and mentees using consistent encoder
    combined = pd.concat([mentors_df[features], mentees_df[features]], axis=0)
    encoder = OneHotEncoder(sparse_output=False, handle_unknown='ignore')
    encoder.fit(combined)

    mentor_encoded = encoder.transform(mentors_df[features])
    mentee_encoded = encoder.transform(mentees_df[features])
    
    # Handle potential NaN values
    if np.isnan(mentor_encoded).any() or np.isnan(mentee_encoded).any():
        mentor_encoded = np.nan_to_num(mentor_encoded, nan=0.0)
        mentee_encoded = np.nan_to_num(mentee_encoded, nan=0.0)

    # Create encoded DataFrames
    encoded_mentors = pd.DataFrame(mentor_encoded, columns=[f'feature_{i}' for i in range(mentor_encoded.shape[1])])
    encoded_mentees = pd.DataFrame(mentee_encoded, columns=[f'feature_{i}' for i in range(mentee_encoded.shape[1])])
    
    # Add back original features and User_ID
    encoded_mentors['User_ID'] = mentors_df['User_ID'].values
    encoded_mentees['User_ID'] = mentees_df['User_ID'].values
    
    for feature in features:
        encoded_mentors[feature] = mentors_df[feature].values
        encoded_mentees[feature] = mentees_df[feature].values
...
# This is the function that was missing - renamed to match the call in kmeans_cluster_matching
def calculate_similarities(mentor_features, mentee_features):
    """
    Calculate two-sided similarity between mentors and mentees
    This is the implementation of two-sided similarity calculation
    """
    features = [col for col in mentor_features.columns if col.startswith('feature_')]
    
    mentor_array = mentor_features[features].values
    mentee_array = mentee_features[features].values
    
    # Compute feature-level weights for more nuanced similarity
    feature_importance = np.ones(mentor_array.shape[1])  # Default equal weights
    
    # Apply weights to feature dimensions
    weighted_mentor_array = mentor_array
    weighted_mentee_array = mentee_array
    
    # Calculate distances
    try:
        mentee_to_mentor_distances = euclidean_distances(weighted_mentee_array, weighted_mentor_array)
        
        # Normalize distances to similarities [0, 1]
        max_distance = np.max(mentee_to_mentor_distances)
        if max_distance > 0:
            similarity_matrix = 1 - (mentee_to_mentor_distances / max_distance)
        else:
            similarity_matrix = np.ones((len(mentee_features), len(mentor_features)))
    except Exception as e:
        print(f"Error calculating distances: {e}")
        # Fallback to a default similarity matrix
        similarity_matrix = np.ones((len(mentee_features), len(mentor_features))) * 0.5
    
    return similarity_matrix

def calculate_two_sided_similarities(mentor_features, mentee_features):
    """
    Advanced two-sided similarity calculation with weighted features
    """
    features = [col for col in mentor_features.columns if col.startswith('feature_')]
    ...
    feature_importance = np.array([
        ...
        1.5,  # Mentoring Preferences
        1.3   # Education
    ])
    
    # Ensure feature_importance length matches feature count
    if len(feature_importance) > mentor_array.shape[1]:
        feature_importance = feature_importance[:mentor_array.shape[1]]
    elif len(feature_importance) < mentor_array.shape[1]:
        # Extend with ones if we have more features than weights
        extension = np.ones(mentor_array.shape[1] - len(feature_importance))
        feature_importance = np.concatenate([feature_importance, extension])
    
    # Apply feature weights
    weighted_mentor_array = mentor_array * feature_importance
    weighted_mentee_array = mentee_array * feature_importance
    
    try:
        # Calculate two-sided distances
        mentee_to_mentor_distances = euclidean_distances(weighted_mentee_array, weighted_mentor_array)
        mentor_to_mentee_distances = euclidean_distances(weighted_mentor_array, weighted_mentee_array)
        
        # Normalize distances with safety check
        max_mentee_to_mentor_dist = np.max(mentee_to_mentor_distances) if np.max(mentee_to_mentor_distances) > 0 else 1
        max_mentor_to_mentee_dist = np.max(mentor_to_mentee_distances) if np.max(mentor_to_mentee_distances) > 0 else 1
        
        # Compute similarities with safe normalization
        mentee_to_mentor_sim = 1 - (mentee_to_mentor_distances / max_mentee_to_mentor_dist)
        mentor_to_mentee_sim = 1 - (mentor_to_mentee_distances / max_mentor_to_mentee_dist)
        
        # Option 1: Simple average (like Euclidean algorithm)
        combined_sim = (mentee_to_mentor_sim + mentor_to_mentee_sim.T) / 2
        
        # Option 2: Harmonic mean (better for balanced matching)
        # combined_sim = 2 * (mentee_to_mentor_sim * mentor_to_mentee_sim.T) / (mentee_to_mentor_sim + mentor_to_mentee_sim.T + 1e-8)
        
        # Option 3: Weighted combination (like Deferred Acceptance)
        # combined_sim = 0.7 * mentee_to_mentor_sim + 0.3 * mentor_to_mentee_sim.T
        
        return combined_sim
    except Exception as e:
        print(f"Error in two-sided similarity calculation: {e}")
        # Return default similarity matrix
        return np.ones((len(mentee_features), len(mentor_features))) * 0.5
...
def calculate_exact_matches(mentor_row, mentee_row, features):
    ...
    for f in features:
        try:
            if str(mentor_row[f]) == str(mentee_row[f]) and str(mentor_row[f]).lower() != "unknown":
                matches += 1
        except:
            continue
    return matches

def kmeans_cluster_matching(mentors_df, mentees_df, max_mentees_per_mentor=2, constraints=None, n_clusters=10):
    """
    Improved two-sided K-means matching algorithm with robust error handling
    """
    try:
        # Define core features for matching
        features = ['Position', 'Experience_Level', 'Primary_Language', 'Industry', 'Education']

        # Extract feature columns for clustering (these should be numeric)
        feature_cols = [col for col in mentors_df.columns if col.startswith('feature_')]
        
        # Get feature matrices with NaN handling
        mentor_feature_matrix = mentors_df[feature_cols].values
        mentee_feature_matrix = mentees_df[feature_cols].values
        
        # Handle any potential NaN values
        mentor_feature_matrix = np.nan_to_num(mentor_feature_matrix)
        mentee_feature_matrix = np.nan_to_num(mentee_feature_matrix)
        
        # KMeans clustering on mentors - handle potential errors
        try:
            # Make sure n_clusters doesn't exceed number of samples
            actual_n_clusters = min(n_clusters, len(mentors_df))
            kmeans = KMeans(n_clusters=actual_n_clusters, random_state=42, n_init=10)
            mentor_clusters = kmeans.fit_predict(mentor_feature_matrix)
        except Exception as e:
            print(f"Error in KMeans clustering: {e}")
            # If clustering fails, assign all mentors to a single cluster
            mentor_clusters = np.zeros(len(mentors_df), dtype=int)

        mentors_df = mentors_df.copy()
        mentors_df['Cluster'] = mentor_clusters

        # Calculate two-sided similarity matrix - this is the key to making it two-sided
        try:
            # Use the two-sided similarity function for advanced matching
            similarity_matrix = calculate_two_sided_similarities(mentors_df, mentees_df)
        except Exception as e:
            print(f"Error calculating two-sided similarity matrix: {e}")
            # Fall back to the simpler similarity calculation if the advanced one fails
            similarity_matrix = calculate_similarities(mentors_df, mentees_df)

        mentor_counts = defaultdict(int)
        matches = []

        # For each mentee, find the best available mentor
        for mentee_idx in range(len(mentees_df)):
            mentee_row = mentees_df.iloc[mentee_idx]
            
            try:
                # Find closest cluster for this mentee
                mentee_vector = mentee_feature_matrix[mentee_idx].reshape(1, -1)
                mentee_cluster = kmeans.predict(mentee_vector)[0]
            except Exception as e:
                print(f"Error predicting cluster for mentee {mentee_idx}: {e}")
                # If prediction fails, assign to cluster 0
                mentee_cluster = 0

            # Get mentors in the same cluster
            cluster_mentors = mentors_df[mentors_df['Cluster'] == mentee_cluster]
            
            # If no mentors in this cluster or all at capacity, consider all mentors
            if len(cluster_mentors) == 0 or all(mentor_counts[idx] >= max_mentees_per_mentor for idx in cluster_mentors.index):
                cluster_mentors = mentors_df

            # Get similarity scores for mentors in this cluster
            mentee_similarities = similarity_matrix[mentee_idx]
            
            # Sort mentors by similarity (descending)
            sorted_mentor_indices = np.argsort(-mentee_similarities)
            
            # Try to find a match that satisfies all constraints
            matched = False
            for mentor_idx in sorted_mentor_indices:
                # Skip if mentor is not in our target clusters
                if mentor_idx not in cluster_mentors.index and len(cluster_mentors) > 0:
                    continue
                    
                # Skip if mentor has reached capacity
                if mentor_counts[mentor_idx] >= max_mentees_per_mentor:
                    continue

                # Check if pair meets all constraints
                if not meets_all_constraints(mentors_df, mentees_df, mentor_idx, mentee_idx, constraints):
                    continue

                # Found a valid match
                try:
                    num_similarities = calculate_exact_matches(
                        mentors_df.iloc[mentor_idx], 
                        mentees_df.iloc[mentee_idx],
                        features
                    )
                except Exception as e:
                    print(f"Error calculating similarities: {e}")
                    num_similarities = 0  # Default if calculation fails

                # Add the match
                matches.append({
                    'Mentor_ID': mentors_df.iloc[mentor_idx]['User_ID'],
                    'Mentee_ID': mentees_df.iloc[mentee_idx]['User_ID'],
                    'Similarity_Score': mentee_similarities[mentor_idx] * 100,  # Scale to percentage
        ...
        print(f"Error in kmeans_cluster_matching: {e}")
        traceback.print_exc()
        # Return empty DataFrame with correct columns
        return pd.DataFrame(columns=['Mentor_ID', 'Mentee_ID', 'Similarity_Score', 'Num_Similarities'])
...
def evaluate_matching(matches_df):
    ...
    avg_similarity_score = matches_df['Similarity_Score'].mean()
...
def randomized_matching_experiment(mentors_df, mentees_df, num_randomizations=100, max_mentees_per_mentor=2, constraints=None):
    ...
    for i in range(num_randomizations):
        ...
        matches_df = kmeans_cluster_matching(
            shuffled_mentors, 
            shuffled_mentees, 
