...
def calculate_similarities(mentor_features, mentee_features):
    """Calculate similarity scores between mentors and mentees."""
...
# Add this function before match_mentees_to_mentors
...
def match_mentees_to_mentors(mentors_df, mentees_df, max_mentees_per_mentor=2, constraints=None):
    """
    Match mentees to mentors with paper's ratio (2 mentees per mentor maximum)
    """
        ...
        for mentor_idx in sorted_mentor_indices:
            ...
            if mentor_counts[mentor_idx] < constraints.get('max_mentees_per_mentor', max_mentees_per_mentor):
                ...
                try:
                    ...
                    # Use the improved calculate_exact_matches function
...
def randomized_matching_experiment(mentors_df, mentees_df, num_randomizations=100, max_mentees_per_mentor=2, constraints=None):
    """
    Run the mentor-mentee matching algorithm across multiple randomized datasets.
    
    Args:
        mentors_df: Preprocessed mentors dataframe
        mentees_df: Preprocessed mentees dataframe
        num_randomizations: Number of times to randomize and run the algorithm
        max_mentees_per_mentor: Maximum number of mentees per mentor
        constraints: Dictionary of constraints to apply (optional)
        
    Returns:
        A dictionary containing performance metrics averaged across all randomizations
    """
    ...
    print(f"\nRunning {num_randomizations} randomized iterations of the Euclidean matching algorithm...")
...
def run_experiment(mentors_file, mentees_file, num_randomizations=100, max_mentees_per_mentor=2, 
    ...
    print("\n" + "="*50)
    print(f"EUCLIDEAN ALGORITHM RESULTS ({num_randomizations} RANDOMIZED ITERATIONS)")
    print("="*50)
    ...
    output_path = 'data/euclidean_results.csv'
    if mentor_sample_size != 500 or mentee_sample_size != 1000:
        # This is a scalability test
        output_path = f'data/euclidean_{mentor_sample_size}m_{mentee_sample_size}m.csv'
    elif len(constraints) > 2:
        # This is a constraint test
        constraint_count = len(constraints)
        output_path = f'data/euclidean_constraints_{constraint_count}.csv'
...
if __name__ == "__main__":
    try:
        print("Starting Euclidean mentor-mentee matching experiment...")
